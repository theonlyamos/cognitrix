...
def _load_connection_status():
    ...
    try:
        if _status_file_path.exists():
            with open(_status_file_path) as f:
...
def update_connection_status(server_name: str, connected: bool, info: dict[str, Any] | None = None):
...
def clear_connection_status(server_name: str | None = None):
