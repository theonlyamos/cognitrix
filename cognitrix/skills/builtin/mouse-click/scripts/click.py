#!/usr/bin/env python3
"""Perform a left mouse click at specified coordinates using pyautogui.
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python click.py <x> <y>", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        print(f"Error clicking at ({x}, {y}): {e}", file=sys.stderr)
        sys.exit(1)
