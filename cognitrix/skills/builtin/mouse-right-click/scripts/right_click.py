#!/usr/bin/env python3
"""Perform a right mouse click at specified coordinates using pyautogui.

Usage:
    python right_click.py <x> <y>
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python right_click.py <x> <y>", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        print(f"Error right-clicking at ({x}, {y}): {e}", file=sys.stderr)
        sys.exit(1)
