...
# get_current_user is defined below get_auth_context — it now accepts both
# session JWTs and API keys (scope checks live in crud_scope/require).
...
async def get_current_user(ctx: 'AuthContext' = Depends(get_auth_context)) -> User:
    """Back-compat user dependency. Accepts JWT or API key; password already
    stripped. Scope enforcement happens in crud_scope/require, not here."""
    return ctx.user
