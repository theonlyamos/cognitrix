...
@pytest.mark.asyncio
async def test_legacy_session_delete_wrapper_fails_closed_without_owner(
    monkeypatch, artifact_store
):
    from cognitrix import artifacts
    ...
    # Session-id-only cleanup has no deletion authority. Both image and
    # document cleanup require an exact owner binding.
    assert deleted == []
