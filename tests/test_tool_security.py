...
async def test_delegated_agent_keeps_allowlists_but_not_document_or_host_authority(
    ...
    class ChildSession:
        ...
        async def get_by_agent_id(cls, *_args, **_kwargs):
            raise AssertionError('delegation must not reuse persisted history')

        async def __call__(self, *args, **kwargs):
            captured.update(kwargs)

    monkeypatch.setattr(Agent, 'find_one', find_one)
    monkeypatch.setattr('cognitrix.sessions.base.Session', ChildSession)
    ...
    assert captured['session_init'] == {
        'agent_id': 'target-agent',
        'user_id': 'user-1',
    }
    assert captured['persist_history'] is False
    assert captured['compact_history'] is False
